...
async function buscarEvolucaoNome() {
  ...
  if (!nome || isNaN(inicio) || isNaN(fim) || inicio > fim) {
    alert("Por favor, preencha corretamente todos os campos e verifique se a década inicial é menor que a final.");
    return;
  }
...
async function compararNomes() {
  const nome1 = document.getElementById("nome2a").value.trim();
  const nome2 = document.getElementById("nome2b").value.trim();
  ...
  const [resp1, resp2] = await Promise.all([
    fetch(`https://servicodados.ibge.gov.br/api/v2/censos/nomes/${nome1}`),
    fetch(`https://servicodados.ibge.gov.br/api/v2/censos/nomes/${nome2}`)
  ]);
  ...
  if (window.graficoComparacaoChart) window.graficoComparacaoChart.destroy();
  ...
  window.graficoComparacaoChart = new Chart(ctx, {
    ...
    data: {
      labels,
      datasets: [
        { label: nome1, data: dados1, borderColor: "#ff6384", borderWidth: 2 },
        { label: nome2, data: dados2, borderColor: "#36a2eb", borderWidth: 2 }
      ],
    },
    options: {
      responsive: true,
